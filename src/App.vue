...
export default class App extends Vue {
  ...
  created(): void {
    this.chartStyle = {};
    this.customSketchers = new Map([['HLine', new HLineSketcher()]]);

    this.mainDs = new DataSource([
      { id: 'hlocv1', title: 'timeseries1', type: 'hlocv', data: [], locked: false, visible: true },
      { id: 'hline1', title: 'hline1', type: 'HLine', data: { def: -0.5, style: { lineWidth: 1, fill: 0, color: '#AA0000' } }, locked: true, visible: true },
      { id: 'hline2', title: 'hline2', type: 'HLine', data: { def: -0.25, style: { lineWidth: 2, fill: 1, color: '#00AA00' } }, locked: false, visible: true },
      { id: 'hline3', title: 'hline3', type: 'HLine', data: { def: 0, style: { lineWidth: 3, fill: 2, color: '#0000BB' } }, locked: false, visible: true },
      { id: 'hline4', title: 'hline4', type: 'HLine', data: { def: 0.25, style: { lineWidth: 4, fill: 3, color: '#AA00BB' } }, locked: false, visible: true },
      { id: 'hline5', title: 'hline5', type: 'HLine', data: { def: 0.5, style: { lineWidth: 5, fill: 4, color: '#AA00BB' } }, locked: false, visible: true },
      { id: 'VLine1', title: 'vline1', type: 'VLine', data: { def: 0, style: { lineWidth: 3, fill: 2, color: '#AABBCC' } }, locked: false, visible: true },
    ]);
  }

  mounted(): void {
    const chartApi: ChartController = this.chart.getChartAPI();
    chartApi.createPane(this.mainDs, { id: 'mainPane' }); // use provided
    chartApi.clearHistory();

    // const newId: DrawingId = this.mainDs.getNewId('HLine');
    // this.mainDs.beginTransaction();
    // this.mainDs.add({ id: newId, title: 'hline6', type: 'HLine', data: { def: -0.75, style: { lineWidth: 1, fill: 0, color: '#AA0000' } }, locked: false, visible: true });
    ...
    // this.mainDs.endTransaction();
    //
    // const secondPane = chartApi.createPane(new DataSource([{ id: 'hline1', title: 'hline1', type: 'HLine', data: { def: -0.75, style: { lineWidth: 1, fill: 0, color: '#AA0000' } }, locked: false, visible: true }])); // use autogenerated id
    // chartApi.togglePane(secondPane);
    // const thirdPane = chartApi.createPane(new DataSource([{ id: newId, title: 'hline6', type: 'HLine', data: { def: -0.75, style: { lineWidth: 1, fill: 0, color: '#AA0000' } }, locked: false, visible: true }]));
    //
    // chartApi.removePane(thirdPane);
    // chartApi.togglePane(secondPane);
